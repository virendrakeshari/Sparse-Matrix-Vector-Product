...
double *y_openmp_product_csr, *y_openmp_product_ellpack;                           //Array to store product result of parallel product computation
...
void matrix_vector_product_CSR_parallel(int num_threads)                             //Function to compute parallel matrix vector product
{    
    int i,j;
    double t;                                              
    y_openmp_product_csr = (double*)malloc(totalRows * sizeof(double));              //Allocating memory for array to store the serial product result
    //Parallelising the execution of matrix vector product
    #pragma omp parallel private(i,j,t) shared(csr_values,csr_column_indices,csr_row_pointer, totalRows, totalColumns, x_vector) num_threads(num_threads)
    {
        for (i = 0; i < totalRows; i++) 
        {   
            ...
            y_openmp_product_csr[i] = t;
            //printf("Thread %d computed row %d\n", tid, i);
        }
        #pragma omp barrier
    }
}
...
void product_results_csr()                                                        //Function to print serial and parallel matrix vector product results
{
    ...
    for (i = 0; i < totalRows; ++i) 
    {
        printf("%.6f  ", y_openmp_product_csr[i]);
    }
     printf("]\n");
}

void compare_results_csr()                                                           //Function to compare serial and parallel matrix vector product results
{
    int i;
    printf("\nComparing the results of serial and parallel Sparse Matrix vector product for CSR\n");
    for (i = 0; i < totalColumns; i++) 
    {
        if (y_serial_product_csr[i] != y_openmp_product_csr[i]) 
        {
            printf("Arrays are different at index %d: y_serial_product_csr[%d] = %.6f, y_openmp_product_csr[%d] = %.6f\n", i, i, y_serial_product_csr[i], i, y_openmp_product_csr[i]);
        }
    }
...
void calculate_max_nonZeros_in_any_row()                                     //Function to calculate Maximum non-Zeros in any row
{
    ...
    //Iterate for all nonZero coefficients
    for (i = 0; i < total_nonZeros; i++) 
    {
...
void matrix_vector_product_ellpack_parallel(int num_threads)                          //Function to compute parallel matrix vector product
{
    int i, j;
    double t;                                                   
    y_openmp_product_ellpack = (double*)malloc(totalRows * sizeof(double));                   //Allocating memory for array to store the serial product result
    //Parallelising the execution of matrix vector product
    #pragma omp parallel num_threads(num_threads) shared(ellpack_values,ellpack_column_indices, totalRows, totalColumns, x_vector) private(i,j,t)
    {
        for (i = 0; i < totalRows; i++) 
        {
            //int tid = omp_get_thread_num();
...
void product_results_ellpack()                                                        //Function to print serial and parallel matrix vector product results
{
    ...
    for (i = 0; i < totalRows; ++i) 
    {
        printf("%.6f  ", y_openmp_product_ellpack[i]);
    }
     printf("]\n");
}

void compare_results_ellpack()                                                           //Function to compare serial and parallel matrix vector product results
{
    int i;
    printf("\nComparing the results of serial and parallel Sparse Matrix vector product for Ellpack storage\n");
    for (i = 0; i < totalColumns; i++) 
    {
        if (y_serial_product_ellpack[i] != y_openmp_product_ellpack[i]) 
        {
            printf("Arrays are different at index %d: y_serial_product_ellpack[%d] = %.6f, y_openmp_product_ellpack[%d] = %.6f\n", i, i, y_serial_product_ellpack[i], i, y_openmp_product_ellpack[i]);
        }
    }
    printf("Results are identical\n");
}

void compare_results_ellpack_csr()                                                           //Function to compare serial and parallel matrix vector product results
{
    int i;
    printf("\nComparing the results of Sparse Matrix vector product for Ellpack and CSR storage formats\n");
    for (i = 0; i < totalColumns; i++) 
    {
        if (y_serial_product_ellpack[i] != y_serial_product_csr[i]) 
        {
            printf("Arrays are different at index %d: y_serial_product_ellpack[%d] = %.6f, y_serial_product_csr[%d] = %.6f\n", i, i, y_serial_product_ellpack[i], i, y_serial_product_csr[i]);
        }
    }
    printf("Results are identical\n");
}
...
{
    struct timespec start_time, end_time, serial_start_csr, serial_end_csr, parallel_start_csr, parallel_end_csr, serial_start_ellpack, serial_end_ellpack, parallel_start_ellpack, parallel_end_ellpack;   //Using struct timespec to compute time for Sparse matrix vector product
    ...
    int num_iterations = 1000;                                                     //Setting number of iterations to obtain average time per kernel invocation
    int num_threads_values[] = {1,2,4,8,12,16};                                    //get number of threads for parallelisation from command line as second argument
    int num_threads_count = 6;

    //int num_threads = atoi(argv[2]);                                              //get number of threads for parallelisation from command line as second argument
    //omp_set_num_threads(num_threads);                                              //set number of threads

    clock_gettime(CLOCK_MONOTONIC, &start_time);                                  //Starting program time
    
    readinputfile(argv[1]);                                                       //Calling function to read input file given as command line argument
    ...
    for (int i = 0; i < num_iterations; i++)                                       //Iterating the product function to obtain multiple time values
    {
        clock_gettime(CLOCK_MONOTONIC, &serial_start_csr);                          // start timer
        matrix_vector_product_CSR();                                                // call function for matrix vector product
        ...
        serial_time_sec_csr = (double)serial_time_ns_csr / 1000000000.0;               //Calculating serial product time in seconds
    ...
    printf("Serial product Performance : %.15g GFLOPS\n", (FLOPS_serial_csr/1000000000)); 
    
    for (int i = 0; i < num_threads_count; i++)
    {
        int num_threads = num_threads_values[i];                                            //Assigning different num_threads value for each set of iterations
        omp_set_num_threads(num_threads);
        //Undertake parallel matrix vector product
        printf("\nUndertaking parallel Sparse matrix vector Product for CSR storage with %d threads using Open MP\n", num_threads);
        printf("\nResults for parallel computation \n");
        for (int i = 0; i < num_iterations; i++) 
        {
            clock_gettime(CLOCK_MONOTONIC, &parallel_start_csr); // start timer
            matrix_vector_product_CSR_parallel(num_threads); // call function with the same array
            clock_gettime(CLOCK_MONOTONIC, &parallel_end_csr); // stop timer
    ...
    compare_results_csr();                                                 //Calling function to compare results for serial and parallel product computation
    ...
    for (int i = 0; i < num_iterations; i++) 
    {
        ...
        matrix_vector_product_ellpack();                                        // call function for serial matrix vector product
        ...
        serial_time_sec_ellpack = (double)serial_time_ns_ellpack / 1000000000.0;                    //Calculating serial product time in seconds
    ...
    for (int i = 0; i < num_threads_count; i++)
    {
        ...
        omp_set_num_threads(num_threads);
        //Undertake parallel matrix vector product
        printf("\nUndertaking parallel Sparse matrix vector Product for Ellpack storage with %d threads using Open MP\n", num_threads);
        printf("\nResults for parallel computation \n");
        for (int i = 0; i < num_iterations; i++) 
        {
            clock_gettime(CLOCK_MONOTONIC, &parallel_start_ellpack); // start timer
            matrix_vector_product_ellpack_parallel(num_threads); // call function with the same array
            clock_gettime(CLOCK_MONOTONIC, &parallel_end_ellpack); // stop timer
        ...
        //printf(" Ellpack Thread = %d Serial Time = %.9f  %15g FLOPS", num_threads, average_parallel_time_ellpack,(FLOPS_parallel_ellpack/1000000000));
    }
    //product_results_ellpack();                                               //Calling function to print results for serial and parallel product computation
    
    compare_results_ellpack();                                                 //Comparing results of serial and parallel results in Ellpack storage format
    
    compare_results_ellpack_csr();                                              //Comparing ellpack and CSR results
